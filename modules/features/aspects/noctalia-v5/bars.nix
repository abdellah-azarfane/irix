        ...
        irix.apps.noctalia.files."config.toml" = {
          ...
          bar = {
            ...
            main = {
              ...
              thickness = 30;             # bar cross-axis size in pixels (height for horizontal, width for vertical)
              background_opacity = 0.5;   # 0.0 (transparent) to 1.0 (opaque)
              ...
              margin_edge = 5;           # distance from the nearest screen edge (positive values float the bar)
              padding = 10;               # main-axis padding from bar edges to start/end widget sections
              ...
              center = ["clock" "cpu-graph" "cpu" "audio visualizer" "temperature" "ram" "weather"];

              # We use the end layout that includes the custom clock-seconds from your snippet
              end = [ "tray" "notifications" "clipboard" "network" "bluetooth"  "battery" "control-center" "session"];

              # [bar.main.monitor.edp1]
              monitor.dp1 = {
